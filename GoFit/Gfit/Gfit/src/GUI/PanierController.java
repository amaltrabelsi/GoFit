   ...
   Callback<TableColumn<Panier, Void>, TableCell<Panier, Void>> cellFactory
    ...
    private void passerCommande(ActionEvent event) {
        ...
        if (action.get() == ButtonType.OK) { 
           ...
           totalproduitduclinet=sp.calculerTotalProduits(2);
           
        c.setTotal(totalcommande);
        c.setNb_Produit(totalproduitduclinet);
       c.setMode_Paiement("cash");
       c.setFK_Reg_Id(1);
       c.setFK_Client_Id(2);
    ...
    private void search(ActionEvent event) {
        Panier p = new Panier();
         List Produit = sp.afficher();
           p.setNom_Produit(searchProductByID.getText().toLowerCase());
		List<Panier> listemps = sp.search(p.getNom_Produit());
